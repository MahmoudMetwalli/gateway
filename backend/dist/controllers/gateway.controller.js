...
const updateGateway = async (req, res, next) => {
    ...
    const updateData = {};
    if (gatewayData.name !== undefined)
        updateData.name = gatewayData.name;
    if (gatewayData.status !== undefined)
...
const listGatewayLogs = async (req, res, next) => {
    const { id: gatewayId } = req.params;
    // Check if gateway exists
    const gateway = await gatewayService.getGatewayById(gatewayId);
    if (!gateway) {
...
const listAllLogs = async (req, res, next) => {
