...
const createGateway = async (
  ...
  const serialExists = await gatewayService.serialNumberExists(
    gatewayData.serial_number
  );
  if (serialExists) {
  ...
  const ipExists = await gatewayService.ipv4AddressExists(
    gatewayData.ipv4_address
  );
  if (ipExists) {
...
const updateGateway = async (
  ...
  if (
    gatewayData.ipv4_address &&
    gatewayData.ipv4_address !== existingGateway.ipv4_address
  ) {
    const ipExists = await gatewayService.ipv4AddressExists(
      gatewayData.ipv4_address
    );
  ...
  if (gatewayData.ipv4_address !== undefined)
    updateData.ipv4_address = gatewayData.ipv4_address;
  if (gatewayData.location !== undefined)
    updateData.location = gatewayData.location;

  const updatedGateway = await gatewayService.updateGateway(id!, {
    ...updateData,
  });
...
const attachDevice = async (
  ...
  if (deviceCount >= 10) {
    res
      .status(400)
      .json({
        error: "Gateway already has the maximum number of devices (10)",
      });
    return;
  }
  ...
  if (device.gateway_id && device.gateway_id !== gatewayId) {
    res
      .status(400)
      .json({ error: "Device is already attached to another gateway" });
    return;
  }

  const updatedDevice = await deviceService.attachDeviceToGateway(
    deviceId,
    gatewayId!
  );
...
const gatewayController = {
  createGateway: wrapAsync(createGateway),
  listGateways: wrapAsync(listGateways),
  getGatewayById: wrapAsync(getGatewayById),
  updateGateway: wrapAsync(updateGateway),
  deleteGateway: wrapAsync(deleteGateway),
  attachDevice: wrapAsync(attachDevice),
  detachDevice: wrapAsync(detachDevice),
  listGatewayLogs: wrapAsync(listGatewayLogs),
  listAllLogs: wrapAsync(listAllLogs),
};
